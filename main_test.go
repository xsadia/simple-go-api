...
func TestGetNonExistentProduct(t *testing.T) {
	...
	json.Unmarshal(response.Body.Bytes(), m) // maybe &m

	if m["error"] != "Product not found" {
		t.Errorf(
			"Expected the 'error' key of the response to be set to 'Product not found'. Got '%s'", m["error"],
		)
	}
}
...
func TestCreateProduct(t *testing.T) {
	...
	if m["price"] != 11.22 {
		t.Errorf("Expected price to be 11.22. Got %.2f", m["price"])
	}
